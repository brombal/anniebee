...
$EDIT = $query->param("edit.x");
$ADD = $query->param("add.x");
$DELETE = $query->param("delete.x");
$LIST = $query->param("list.x");
$VIEW = $query->param("view.x");
...
if($MAINACT eq "View") {
  ...
  @readlines = <FORSALEHTML>;
  splice(@readlines, $#readlines - 1, 1, "<p><form action=\"forsale.pl\" method=\"post\"><center><input type=\"submit\" name=\"mainact\" value=\"Return\"></center></form>\n");
  close FORSALEHTML;
  print @readlines;
}
